...
import { Post, validatePartialPost } from '../schemas/PostSchema.js'
import { dataFailure, dataSuccess } from '../structures/Data.js'
import { SanitizedPost } from '../structures/SanitizedPost.js'
...
export class PostController {
  ...
  static async getAll (request: Request, response: Response): Promise<void> {
    ...
    const sanitizedPosts = await SanitizedPost.getFromPosts(posts)

    response.json(dataSuccess(sanitizedPosts))
  }
  ...
  static async getById (request: Request, response: Response): Promise<void> {
    ...
    if (post) {
      const sanitizedPost = await SanitizedPost.getFromPost(post)
      response.json(dataSuccess(sanitizedPost))
    } else {
  ...
  static async create (request: Request, response: Response): Promise<void> {
    ...
    if (postCreated) {
      const sanitizedPost = await SanitizedPost.getFromPost(postCreated)
      response.status(201).json(dataSuccess(sanitizedPost))
    } else {
