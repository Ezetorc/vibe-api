...
import { validateNotification } from '../schemas/notification.schema.js';
...
export class NotificationController {
    ...
    static async create(request, response) {
        ...
        if (!result.success ||
            ...
            response
                .status(400)
                .json(dataFailure(result.error?.toString() ?? 'Error during notification creation'));
            return;
        }
    ...
    static async markAsSeen(request, response) {
        const { notificationsIds } = request.body;
        console.log('notificationsIds: ', notificationsIds);
        const isValidArray = Array.isArray(notificationsIds) &&
